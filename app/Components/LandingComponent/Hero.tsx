  ...
  return (
    <div className="flex flex-col items-center gap-2  p-10 rounded-sm pt-30 ">
      ...
      <div className="relative md:h-70 md:w-70 h-50 w-50  rounded-full overflow-hidden shadow-2xl shadow-amber-50/20 mb-10">
        ...
        <Image fill src={`${data?.image ?? "/personCoding.jpg"}`} alt="" className="object-cover " />
      </div>

      {/* Info Card */}
      <div className=" rounded-3xl  space-y-3 text-center z-40 ">
        <h1 className="md:text-5xl text-2xl font-semibold text-white">{data?.full_name ?? "John Doe"}</h1>
        <h2 className="md:text-3xl text-md font-semibold text-white/90">
